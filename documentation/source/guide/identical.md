...
So as a solution the `identicalCheck` and `identicalRepair` tools were added which scans code for specific patterns and
...
The `IDENTICAL` ones are used for `bash` code while the `DOC TEMPLATE:` pattern is used for comment/arguments which are
identical across functions.
...
The point of the identical matching is to ensure that code matches in two places; after a scan the identical matching
will list tokens which have a single instance only - while it may not be an error, it may prove useful to remove
