# Guides

## {applicationName} Guides

- [Why {applicationName}](./why.md)
- [Coding in Bash](./coding.md)
- [Testing Tools](./testing.md)
- [Environment file formats](./environment-files.md) - Docker vs. Bash-compatible environment files
- [Direct binaries](./bin.md) - Code you can call from the shell directly
- [Guide to types](./types.md) - Data types supported by {applicationName}
- [Function Documentation](./documentation.md) - {applicationName} supports documentation via the comment above a Bash
  function.
- [Identical](./identical.md) - The identical tool
- [Hooks](./hooks.md) - Hooks are called by {applicationName} at various times and allow for extension and customization of
  various behaviors.
- [{applicationName} Encodings](./encodings.md) - Ways things are encoded in text
- [Installer](./installer.md) - [Build your own standalone installer script using {applicationName}](./installer.md)
- [Artifacts](./artifacts.md) - Known artifacts generated by build tools
- [Oddities](./oddities.md) - Odd code explained

## Bash guides

`bash` has been in existence since [1979](https://en.wikipedia.org/wiki/Version_7_Unix) (
see [bash](https://en.wikipedia.org/wiki/Bourne_shell)) and has a long history as a steady and stable shell to bootstrap
your systems.

- [Bash Builtin Documentation](./builtin.md)
- [`FUNCNAME` and `BASH_SOURCE` notes](./notes.md)
- [`test` cheatsheet](./test-cheatsheet.md) - Bash `test` and `[ ]` expressions
- [Trap Manual Page](./trap.md) - Never enjoy digging to find this definition for `trap`.
- [Bash cheatsheet](./bash-cheatsheet.md) - `shopt` meanings
- [Set cheatsheet](./set-cheatsheet.md) - `set` meanings
- [Bash variable cheatsheet](./bash-variable-cheatsheet.md) - `${var#match}` and `${var%match}`
- [strftime cheatsheet](./strftime-cheatsheet.md) - `date` percent formatting codes
- [Bash FAQ](./bash-FAQ.md) - Raw, original document.
- [man read](./man-read.md) - Manual page for `read`
- [man trap](./man-trap.md) - Manual page for `trap`

## History

- [Deprecated functionality](./deprecated.md)
