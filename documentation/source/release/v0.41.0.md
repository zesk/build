...
- Moved `daemontools` service template files into `bin/build/identical` to keep code space `bin/build/tools` clean of
  non-running code
- Console output is now cached in `etc/console.help.txt` for help files, it's around 400K and contains a list of
  terminal console escape codes separated by lines `:isAlign` and `:END isAlign()` - `usageTemplate` uses this to save
  time
- `BUILD_DEBUG` for `usage-profile` added to speed up `usageDocument`
- Probably need to look at how this affects other projects which use Zesk Build as well... later.
