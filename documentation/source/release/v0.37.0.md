...
- Added a third internal IDENTICAL tag `__IDENTICAL__` - used for `bashDocumentation`
- Ensured most help use `# __IDENTICAL bashDocumentation 1`
...
- `bashDocumentation` now maps the text `{identical}` to the capitalized version of such post generation so you can add the word
...
- `BUILD_COLORS` used to support the value `1` and no longer does; use `true` or `false` and if it does not match either
  it will be set automatically upon the first call to `consoleHasColors` (this behavior is unchanged)- ---- identical repair
