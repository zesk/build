...
- `domainName` - The domain name to fetch the certificate for
- `region` - String. Optional. The region to find the certificate in (defaults to `us-east-1`).
...
- `1` - Environment error
- `2` - Argument error
### `awsGetIdentity` - Get the account ID and user ID associated with the

Get the account ID and user ID associated with the Amazon profile in `~/.aws/credentials`

- Location: `bin/infrastructure/tools/aws.sh`

#### Arguments

- `profile` - String. Required. Profile to fetch account ID from.

#### Sample Output

    {
      "UserId": "AIDA2V5AGTUB5GWSEEEED",
      "Account": "734238350000",
      "Arn": "arn:aws:iam::734238350000:user/robot-devops"
    }
