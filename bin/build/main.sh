...
--interrupt)inArray INT "${traps[@]+"${traps[@]}"}"||traps+=("INT")
;;
--error)inArray ERR "${traps[@]+"${traps[@]}"}"||traps+=("ERR")
;;
...
;;
--name)shift||throwArgument "$handler" "missing $argument argument"||return $?
...
--tail)endBinary="tail"
;;
--symbol)shift||throwArgument "$handler" "missing $argument argument"||return $?
symbol="$1"
;;
...
__exitCode=0
case "$__cmd" in
"."|" "|""|$'\r')__cmd="${__BUILD_BASH_DEBUG_LAST:0:1}"
if [ -z "$__cmd" ];then
...
return 1
;;
"j")decorate warning "Skipping $BASH_COMMAND"
__BUILD_BASH_DEBUG_LAST="jump"
return 2
;;
"d"|"i")statusMessage printf -- ""
__BUILD_BASH_DEBUG_LAST="into"
return 0
;;
"k")statusMessage --last decorate info "Call stack:"
debuggingStack
return 1
;;
"s"|"n")__bashDebugCommandStep
statusMessage printf -- ""
__BUILD_BASH_DEBUG_LAST="step"
return 0
;;
...
return 1
;;
"u")statusMessage printf -- ""
__bashDebugCommandUnwatch
return 1
;;
"w")statusMessage printf -- ""
__bashDebugCommandWatch
return 1
;;
"q")statusMessage --last decorate notice "Debugger control ended."
bashDebuggerDisable
return 3
;;
"!")statusMessage printf -- ""
...
case "$type" in
Flag)argumentName="$(_commentArgumentName "$spec" "$stateFile" "$__index" "$argument")"||returnClean "$?" "${clean[@]}"||return $?
...
rm -rf "${argumentDirectory:?}/*"||:)||throwEnvironment "$handler" "Loading $cacheFile"||return $?
fi
...
local type="${1-}"
case "$type" in
File|FileDirectory|Directory|LoadEnvironmentFile|RealDirectory)printf "%s\n" "$type"
return 0
;;
EmptyString|String|EnvironmentVariable)printf "%s\n" "$type"
return 0
;;
Flag|Boolean|PositiveInteger|Integer|UnsignedInteger|Number)printf "%s\n" "$type"
return 0
;;
Executable|Callable|Function)printf "%s\n" "$type"
return 0
;;
URL)printf "%s\n" "$type"
return 0
...
;;
--signals)shift||throwArgument "$handler" "missing $argument argument"||return $?
...
;;
--timeout)shift||throwArgument "$handler" "missing $argument"||return $?
...
case "$argument" in
--help)"$handler" 0&&return $?||return $?;;
--find)local tempArgs&&shift&&IFS=' ' read -r -a tempArgs <<<"${1-}"||:
...
--help)"$handler" 0&&return $?||return $?;;
--handler)shift&&handler=$(validate "$handler" Function "$argument" "${1-}")||return $?;;
--services)shift||throwArgument "$handler" "missing $argument argument"||return $?
...
case "$argument" in
--help)"$handler" 0&&return $?||return $?;;
--pattern)[ -z "$filePattern" ]||throwArgument "$handler" "$argument already specified"||return $?
shift
...
case "$argument" in
--help)"$handler" 0&&return $?||return $?;;
--header)shift&&local name="${1%%:}" value="${1#*:}"
...
binary=("$tempBin")
;;
--argument-format)shift&&format=$(validate "$handler" String "$argument" "${1-}")||return $?
...
--user)shift&&user=$(validate "$handler" String "$argument (user)" "$user")||return $?
if [ "$user" != "${user#*:}" ];then
userHasColons=true
fi
...
;;
--debug)debugFlag=true;;
--timeout)shift&&timeoutSeconds=$(validate "$handler" PositiveInteger "$argument" "${1-}")||return $?
;;
--agent)local agent
...
case "$format" in
wget)wgetArgs+=(--max-redirect "$maxRedirections" -q)
catchEnvironment "$handler" "${binary[@]}" --output-document="$target" "${wgetArgs[@]+"${wgetArgs[@]}"}" "$url" "$@"||return $?
;;
curl)curlArgs+=(-L --max-redirs "$maxRedirections" -s -f --no-show-error)
...
BUILD_COLORS=false
case "${TERM-}" in ""|"dumb"|"unknown")BUILD_COLORS=true;;*)local termColors
...
case "${1-}" in
--delete)[ -f "${__BUILD_DECORATE_THEME-}" ]||return 0
...
case "$argument" in
--help)"$handler" 0&&return $?||return $?;;
--fill)shift||throwArgument "$handler" "missing $argument argument"||return $?
[ 1 -eq "${#1}" ]||throwArgument "$handler" "Fill character must be single character"||return $?
fill="$1"
width="${width:-needed}"
;;
--width)shift||throwArgument "$handler" "missing $argument argument"||return $?
...
[ -n "$argument" ]||throwArgument "$handler" "blank #$__index/$__count ($(decorate each quote -- "${__saved[@]}"))"||return $?
case "$argument" in
--require)requireHook=true
;;
--source)sourceHook=true
;;
--)shift
break
;;
...
case "$argument" in
--help)"$handler" 0&&return $?||return $?;;
--reset)rm -rf "$cacheDirectory"||:
return 0
;;
*[^-_a-zA-Z0-9]*)throwArgument "$handler" "Invalid argument or variable name: $argument"||return $?
;;
*)if
isInteger "$argument"
then
...
--writer)[ -z "$namedPipe" ]||throwArgument "$handler" "No namedPipe supplied"||return $?
...
done)|catchEnvironment "$handler" sort -u||return $?
}
...
;;
"mariadb-server")printf "%s\n" mariadb-common mariadb-server
;;
"mysql")printf "%s\n" mysql-client
;;
"mysql-server")printf "%s\n" mysql-server
;;
"sha1sum")printf "%s\n" "coreutils"
;;
*)printf "%s\n" "$1"
...
[ "${1-}" != "--help" ]||__help "$handler" "$@"||return 0
case "${1-}" in
":"|"."|"["|"alias"|"bg"|"bind"|"break"|"builtin"|"case"|"cd"|"command"|"compgen"|"complete"|"continue"|"declare"|"dirs"|"disown"|"echo"|"enable"|"eval"|"exec"|"exit"|"export"|"fc"|"fg"|"getopts")return 0
;;
"hash"|"help"|"history"|"if"|"jobs"|"kill"|"let"|"local"|"logout"|"popd"|"printf"|"pushd"|"pwd"|"read"|"readonly"|"return"|"set"|"shift"|"shopt"|"source"|"suspend"|"test"|"times"|"trap"|"type"|"typeset")return 0
;;
"ulimit"|"umask"|"unalias"|"unset"|"until"|"wait"|"while")return 0
;;
*)return 1
esac
}
...
case "$argument" in
--help)"$handler" 0&&return $?||return $?;;
--background)xtermCode="11"
;;
--foreground)xtermCode="10"
;;
*)throwArgument "$handler" "unknown #$__index/$__count \"$argument\" ($(decorate each code -- "${__saved[@]}"))"||return $?
esac
shift
...
case "$argument" in
--help)"$handler" 0&&return $?||return $?;;
--env-file)[ -z "$rootEnv" ]||throwArgument "$handler" "$argument already"||return $?
shift
rootEnv=$(validate "$handler" File "rootEnv" "$1")||return $?
;;
--mapper)[ -z "$environmentMapper" ]||throwArgument "$handler" "$argument already"||return $?
shift
...
case "$name" in
"button returned")button="$value"
;;
"gave up")isBoolean "$value"||throwEnvironment "$handler" "gave up should be a boolean: $value"||return $?
...
printf "%s\n" "$button")||returnClean $? "$quietErrors"||return $?
}
_darwinDialog(){
usageDocument "${BASH_SOURCE[0]}" "${FUNCNAME[0]#_}" "$@"
}
...
case "$argument" in
--help)"$handler" 0&&return $?||return $?;;
--timeout)shift||throwArgument "$handler" "missing $argument argument"||return $?
...
local bootPid
bootPid="$(nohup svscanboot 2>/dev/null&
printf -- "%d" $!)"
...
--deploy)! $cleanupFlag&&! $revertFlag||throwArgument "$handler" "$argument is incompatible with --cleanup and --revert"||return $?
cleanupFlag=false
revertFlag=false
;;
...
buildTarget="$1"
;;
--skip-ssh-host)addSSHHosts=false
;;
--add-ssh-host)addSSHHosts=true
;;
--home)shift
...
ee+=("$argument" "$envPair")
;;
--env-file)local envFile tempEnv
shift
...
;;
--clean)deleteVolumes=true
;;
--keep)deleteVolumes=false
keepVolumes=true
;;
--build)command="build"
;;
--arg|--default-env|--env)local environmentPair
shift
...
;;
--secure-defaults)read -d "" -r -a secureList < <(environmentSecureVariables)||:
;;
--ignore)shift
...
--variables)shift&&local listText&&listText="$(validate "$handler" "CommaDelimitedList" "$__argument" "${1-}")"||return $?
...
catchReturn "$handler" environmentOutput "${aa[@]+"${aa[@]}"}" "${variables[@]+"${variables[@]}"}" >>"$tempEnv.after"||returnClean $? "${clean[@]}"||return $?)||returnClean $? "${clean[@]}"||return $?
if $__debugFlag;then
...
case "$name" in [^[:alpha:]_]*|*[^[:alnum:]_]]*)returnArgument "Invalid name at line $index: $name"||result=$?
;;
...
;;
--last)appendLast=true
;;
--open-links)openLinks=true
;;
*)comment="$*"
break
esac
...
case "$branch" in
main|staging)throwEnvironment "$handler" "Already in branch $(decorate code "$branch")"||return $?
;;
HEAD)throwEnvironment "$handler" "Ignore branches named $(decorate code "$branch")"||return $?
;;
*)returnCode=0
...
--verbose)verbose=true
;;
--application)shift||throwArgument "$handler" "missing $argument argument"||return $?
...
--debug)debugFlag=true
;;
--global)flags+=("$argument")
;;
install|run|update|uninstall)[ -z "$action" ]||throwArgument "$handler" "Only a single action allowed: $argument (already: $action)"
action="$argument"
;;
-*)throwArgument "$handler" "unknown #$__index/$__count \"$argument\" ($(decorate each code -- "${__saved[@]}"))"||return $?
;;
...
case "$action" in
run)! $globalFlag||throwArgument "$handler" "--global makes no sense with run"||return $?
printf "%s\n" "run"
;;
update)printf "%s\n" "install"
;;
...
case "$argument" in
--help)"$handler" 0&&return $?||return $?;;
--tag|--no-tag|--skip-tag)statusMessage decorate subtle "$argument is deprecated"
;;
--composer)shift
...
--quiet)quietFlag=true
;;
--docker)decorate warning "Requiring docker composer"
forceDocker=true
;;
...
case "$argument" in
--help)"$handler" 0&&return $?||return $?;;
--skip-backup|--no-backup)backupFlag=false
;;
--verbose)verbose=true
verboseArgs=("-v")
;;
...
assertFileContains(){
__testLoader "_${FUNCNAME[0]}" __assertFileContainsThis --line-depth 2 "$@"||return $?
}
_assertFileContains(){
usageDocument "${BASH_SOURCE[0]}" "${FUNCNAME[0]#_}" "$@"
}
assertFileDoesNotContain(){
__testLoader "_${FUNCNAME[0]}" __assertFileDoesNotContainThis "$@"||return $?
}
...
assertLessThan(){
__testLoa "_${FUNCNAME[0]}" _assertNumericHelper "$@" -lt||return $?
}
_assertLessThan(){
usageDocument "${BASH_SOURCE[0]}" "${FUNCNAME[0]#_}" "$@"
}
...
case "$argument" in
--help)"$handler" 0&&return $?||return $?;;
--install)catchReturn "$handler" packageWhich ifconfig net-tools||return $?
;;
*)throwArgument "$handler" "unknown #$__index/$__count \"$argument\" ($(decorate each code -- "${__saved[@]}"))"||return $?
esac
shift
...
local pid
pid=$(wget "${aa[@]}" "$url" 2>&1|tee "$logFile"|grep -E '^--' >"$progressFile"&
printf "%d" $!)||returnClean $? "$logFile"||return $?
...
;;
--scale)aspectRatio=false
;;
--preserve-aspect-ratio)aspectRatio=true
;;
...
case "$argument" in
--help)"$handler" 0&&return $?||return $?;;
--skip-errors)skipColorErrors=true
;;
--fill)fillMissing=true
;;
--verbose|-v)verboseFlag=true
;;
--ignore|-i)ignoreErrors=true;;
-*)throwArgument "$handler" "unknown #$__index/$__count \"$argument\" ($(decorate each code -- "${__saved[@]}"))"||return $?
;;
...
case "$result" in 0|1)! $verboseFlag||statusMessage decorate info "Requesting attention: $result"
_iTerm2_setValue RequestAttention "$((result-1))"
didSomething=true
;;
*)case "$argument" in
"start")_iTerm2_setValue RequestAttention 1
didSomething=true
;;
"stop")_iTerm2_setValue RequestAttention 1
didSomething=true
;;
"!"|"fireworks")! $verboseFlag||statusMessage decorate info "Requesting fireworks"
...
catchReturn "$handler" mapEnvironment <"$source" >"$target"||return $?)||return $?
}
export BUILD_HOME
