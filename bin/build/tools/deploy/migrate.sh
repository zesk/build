...
__deployMigrateDirectoryToLink() {
  ...
  appVersion=$(deployApplicationVersion "$applicationPath") || returnThrowEnvironment "$handler" "No application deployment version" || return $?
  if [ -L "$applicationPath" ]; then
    printf "%s %s %s\n" "$(decorate code "$applicationPath")" "$(decorate success "is already a link to")" "$(decorate red "$appVersion")"
    return 0
  fi
  deployHasVersion "$deployHome" "$appVersion" || returnThrowEnvironment "$handler" "Application version $appVersion not found in $deployHome" || return $?

  [ ! -d "$deployHome/$appVersion/app" ] || returnThrowEnvironment "$handler" "Old app directory $deployHome/$appVersion/app exists, stopping" || return $?
  hookRunOptional --application "$applicationPath" maintenance on || returnThrowEnvironment "$handler" "Unable to enable maintenance" || return $?
  ...
  if ! deployLink "$tempAppLink" "$deployHome/$appVersion/app"; then
    ...
    returnThrowEnvironment "$handler" "deployLink failed" || return $?
  fi
  # Now move our folder and the link to where the folder was in one fell swoop
  # or mv -hf
  catchEnvironment "$handler" mv -f "$applicationPath" "$deployHome/$appVersion/app" || returnThrowEnvironment "$handler" "Unable to move live application from $applicationPath to $deployHome/$appVersion/app" || return $?

  if ! catchEnvironment "$handler" mv -f "$tempAppLink" "$applicationPath"; then
    # Like really? Like really? Something is likely F U B A R
    if ! catchEnvironment "$handler" mv -f "$deployHome/$appVersion/app" "$applicationPath"; then
    ...
    returnThrowEnvironment "$handler" "Unable to move live link $tempAppLink -> $applicationPath" || return $?
  fi
