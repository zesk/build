...
__deployMigrateDirectoryToLink() {
  ...
  appVersion=$(deployApplicationVersion "$applicationPath") || throwEnvironment "$handler" "No application deployment version" || return $?
  if [ -L "$applicationPath" ]; then
    printf "%s %s %s\n" "$(decorate code "$applicationPath")" "$(decorate success "is already a link to")" "$(decorate red "$appVersion")"
    return 0
  fi
  deployHasVersion "$deployHome" "$appVersion" || throwEnvironment "$handler" "Application version $appVersion not found in $deployHome" || return $?

  [ ! -d "$deployHome/$appVersion/app" ] || throwEnvironment "$handler" "Old app directory $deployHome/$appVersion/app exists, stopping" || return $?
  hookRunOptional --application "$applicationPath" maintenance on || throwEnvironment "$handler" "Unable to enable maintenance" || return $?
  ...
  if ! deployLink "$tempAppLink" "$deployHome/$appVersion/app"; then
    ...
    throwEnvironment "$handler" "deployLink failed" || return $?
  fi
  # Now move our folder and the link to where the folder was in one fell swoop
  # or mv -hf
  catchEnvironment "$handler" mv -f "$applicationPath" "$deployHome/$appVersion/app" || throwEnvironment "$handler" "Unable to move live application from $applicationPath to $deployHome/$appVersion/app" || return $?
  ...
  if ! catchEnvironment "$handler" mv -f "$tempAppLink" "$applicationPath"; then
    ...
    throwEnvironment "$handler" "Unable to move live link $tempAppLink -> $applicationPath" || return $?
  fi
