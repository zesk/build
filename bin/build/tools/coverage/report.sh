...
__bashCoverageReport() {
  ...
  decorate pair "Cache" "$reportCache"
  decorate pair "Report" "$target"
  local file
  if [ "${#files[@]}" -eq 0 ]; then
  ...
  for file in coverage.css coverage.js; do
    catchEnvironment "$handler" cp "$(__bashCoverageReportTemplate "$file")" "$target/$file" || return $?
  done
...
__bashCoverageReportProcessStats() {
  ...
  catchEnvironment "$handler" sort -u <"$reportCache/all" >"$reportCache/files" || return $?
}
...
__bashCoverageReportConvertFiles() {
  ...
  while read -r file; do
    ...
    catchEnvironment "$handler" rm -rf "$targetFile.$$" || return $?
