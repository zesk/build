...
__identicalFindTokens() {
  ...
  [ "${#prefixes[@]}" -gt 0 ] || throwArgument "$handler" "Need at least one prefix" || return $?
  [ "${#files[@]}" -gt 0 ] || throwArgument "$handler" "Need at least one file" || return $?
...
__identicalWatchLoop() {
  ...
  while $running; do
    ...
    statusMessage decorate info "$(decorate subtle "$(date +%T)"): Sleeping for $(localePluralWord "$elapsed" second) $(decorate file "$lastFile") modified $(__identicalWatchDecorateDate "$lastTimestamp") ..."
