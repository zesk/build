#!/usr/bin/env bash
# Copyright &copy; 2026 Market Acumen, Inc.
# Generated on 2026-06-20
# shellcheck disable=SC2034
argument="none"
base="iterm2.sh"
derivations=([0]="return_code" [1]="fn" [2]="lowerFn" [3]="fnMarker" [4]="argument" [5]="usage")
description=$'Colors for various UI elements\n\n'
descriptionLineCount="2"
file="bin/build/tools/iterm2.sh"
fn="iTerm2ColorTypes"
fnMarker="iterm2colortypes"
foundNames=()
line="240"
original="iTerm2ColorTypes"
rawComment=$'Colors for various UI elements\n\n'
return_code=$'0 - Success\n1 - Environment error\n2 - Argument error\n'
sourceFile="bin/build/tools/iterm2.sh"
sourceHash="04b5b95900782435a2524d52704ddd8201c9d15c"
sourceLine="240"
summary="Colors for various UI elements"
summaryComputed="true"
usage="iTerm2ColorTypes"
# shellcheck disable=SC2016
helpConsole=''$'\e''[[(label)]mUsage'$'\e''[0m: '$'\e''[[(info)]miTerm2ColorTypes'$'\e''[0m'$'\n'''$'\n''Colors for various UI elements'$'\n'''$'\n''Return codes:'$'\n''- '$'\e''[[(code)]m0'$'\e''[[(reset)]m - Success'$'\n''- '$'\e''[[(code)]m1'$'\e''[[(reset)]m - Environment error'$'\n''- '$'\e''[[(code)]m2'$'\e''[[(reset)]m - Argument error'
# shellcheck disable=SC2016
helpPlain='Usage: iTerm2ColorTypes'$'\n'''$'\n''Colors for various UI elements'$'\n'''$'\n''Return codes:'$'\n''- 0 - Success'$'\n''- 1 - Environment error'$'\n''- 2 - Argument error'
documentationPath="documentation/source/tools/iterm2.md"
