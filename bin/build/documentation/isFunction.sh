...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]misFunction [[(magenta)]mstring'$'\n'''$'\n''    [[(red)]mstring  [[(value)]mRequired. String to test if it is a bash function. Builtins are supported. [[(code)]m. is explicitly not supported to disambiguate it from the current directory [[(code)]m.'$'\n'''$'\n''Test if argument are bash functions'$'\n''If no arguments are passed, returns exit code 1.'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m0 - argument is bash function'$'\n''- [[(code)]m1 - argument is not a bash function'$'\n'''
# elapsed 3.202
