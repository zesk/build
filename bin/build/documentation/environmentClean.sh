...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]menvironmentClean [[(blue)]m[ keepEnvironment ]'$'\n'''$'\n''    [[(blue)]mkeepEnvironment  [[(value)]mEnvironmentVariable. Optional. Keep this environment variable. ZeroOrMore.[[(reset)]m'$'\n'''$'\n''Clean [[(cyan)]mmost[[(reset)]m exported variables from the current context except a few important ones:'$'\n''- BUILD_HOME PATH LD_LIBRARY USER HOME PS1 PS2 PS3 PS4'$'\n''Calls unset on any variable in the global environment and exported.'$'\n''Use with caution. Any additional environment variables you wish to preserve, simply pass those on the command line'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m0[[(reset)]m - Success'$'\n''- [[(code)]m1[[(reset)]m - Environment error'$'\n''- [[(code)]m2[[(reset)]m - Argument error'$'\n'''
# elapsed 3.574
