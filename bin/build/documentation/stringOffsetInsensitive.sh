...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]mstringOffsetInsensitive [[(bold)]m[[(magenta)]mneedle [[(bold)]m[[(magenta)]mhaystack'$'\n'''$'\n''    [[(red)]mneedle    [[(value)]mString. Required.'$'\n''    [[(red)]mhaystack  [[(value)]mString. Required.'$'\n'''$'\n''Outputs the integer offset of [[(code)]mneedle if found as substring in [[(code)]mhaystack (case-insensitive)'$'\n''If [[(code)]mhaystack is not found, -1 is output'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m0 - Success'$'\n''- [[(code)]m1 - Environment error'$'\n''- [[(code)]m2 - Argument error'$'\n'''$'\n''Writes to [[(code)]mstdout:'$'\n''[[(code)]mInteger. The offset at which the [[(code)]mneedle was found in [[(code)]mhaystack. Outputs -1 if not found.'$'\n'''
