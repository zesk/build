...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]mfileLineCount [[(blue)]m[ --help ] [[(blue)]m[ --handler handler ] [[(blue)]m[ file ]'$'\n'''$'\n''    [[(blue)]m--help             [[(value)]mFlag. Optional. Display this help.'$'\n''    [[(blue)]m--handler handler  [[(value)]mFunction. Optional. Use this error handler instead of the default error handler.'$'\n''    [[(blue)]mfile               [[(value)]mFile. Optional. Output line count for each file specified. If no files specified, uses stdin.'$'\n'''$'\n''Outputs the number of lines read from stdin (or supplied files) until EOF. For multiple files passed on the command line - each one is output separately.'$'\n''This is essentially a wrapper around [[(code)]mwc -l which strips whitespace and does type checking.'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m0 - Success'$'\n''- [[(code)]m1 - Environment error'$'\n''- [[(code)]m2 - Argument error'$'\n'''$'\n''Reads from [[(code)]mstdin:'$'\n''Lines are read from standard in and counted'$'\n'''$'\n''Writes to [[(code)]mstdout:'$'\n''UnsignedInteger'$'\n''[[(code)]mUnsignedInteger'$'\n'''
# elapsed 3.624
