...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]mcatchEnvironmentQuiet [[(bold)]m[[(magenta)]mhandler [[(bold)]m[[(magenta)]mquietLog [[(bold)]m[[(magenta)]mcommand ...'$'\n'''$'\n''    [[(red)]mhandler      [[(value)]mFunction. Required. Failure command[[(reset)]m'$'\n''    [[(red)]mquietLog     [[(value)]mFile. Required. File to output log to temporarily for this command. If [[(code)]mquietLog[[(reset)]m is [[(code)]m-[[(reset)]m then creates a temporary file for the command which is deleted automatically.[[(reset)]m'$'\n''    [[(red)]mcommand ...  [[(value)]mCallable. Required. Thing to run and append output to [[(code)]mquietLog[[(reset)]m.[[(reset)]m'$'\n'''$'\n''Run [[(code)]mhandler[[(reset)]m with an environment error'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m0[[(reset)]m - Success'$'\n''- [[(code)]m1[[(reset)]m - Environment error'$'\n''- [[(code)]m2[[(reset)]m - Argument error'$'\n'''
# elapsed 3.616
