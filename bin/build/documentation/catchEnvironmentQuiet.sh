...
# shellcheck disable=SC2016
helpPlain='Usage: [[(info)]mcatchEnvironmentQuiet [[(bold)]mhandler [[(bold)]mquietLog [[(bold)]mcommand ...'$'\n'''$'\n''    [[(red)]mhandler      Function. Required. Failure command'$'\n''    [[(red)]mquietLog     File. Required. File to output log to temporarily for this command. If [[(code)]mquietLog is [[(code)]m- then creates a temporary file for the command which is deleted automatically.'$'\n''    [[(red)]mcommand ...  Callable. Required. Thing to run and append output to [[(code)]mquietLog.'$'\n'''$'\n''Run [[(code)]mhandler with an environment error'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m0 - Success'$'\n''- [[(code)]m1 - Environment error'$'\n''- [[(code)]m2 - Argument error'$'\n'''
