...
# shellcheck disable=SC2016
helpPlain='Usage: [[(info)]mtrimSpace [[(blue)]m[ text ]'$'\n'''$'\n''    [[(blue)]mtext  [[(value)]mEmptyString. Optional. Text to remove spaces. If no arguments are supplied it is assumed that input should be read from standard input.[[(reset)]m'$'\n'''$'\n''Trim spaces and only spaces from arguments or a pipe'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m0[[(reset)]m - Success'$'\n''- [[(code)]m1[[(reset)]m - Environment error'$'\n''- [[(code)]m2[[(reset)]m - Argument error'$'\n'''$'\n''Reads from [[(code)]mstdin[[(reset)]m:'$'\n''Reads lines from stdin until EOF'$'\n'''$'\n''Writes to [[(code)]mstdout[[(reset)]m:'$'\n''Outputs trimmed lines'$'\n'''$'\n''Example:'$'\n''    trimSpace "$token"'$'\n''    grep "$tokenPattern" | trimSpace > "$tokensFound"'$'\n'''
# elapsed 3.901
