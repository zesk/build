...
# shellcheck disable=SC2016
helpPlain='Usage: trimSpace [[(blue)]m[ text ]'$'\n'''$'\n''    [[(blue)]mtext  EmptyString. Optional. Text to remove spaces. If no arguments are supplied it is assumed that input should be read from standard input.'$'\n'''$'\n''Trim spaces and only spaces from arguments or a pipe'$'\n'''$'\n''Return codes:'$'\n''- 0 - Success'$'\n''- 1 - Environment error'$'\n''- 2 - Argument error'$'\n'''$'\n''Reads from stdin:'$'\n''Reads lines from stdin until EOF'$'\n'''$'\n''Writes to stdout:'$'\n''Outputs trimmed lines'$'\n'''$'\n''Example:'$'\n''    trimSpace "$token"'$'\n''    grep "$tokenPattern" | trimSpace > "$tokensFound"'$'\n'''
# elapsed 2.896
