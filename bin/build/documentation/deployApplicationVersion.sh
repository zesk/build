...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]mdeployApplicationVersion [[(bold)]m[[(magenta)]mapplicationHome'$'\n'''$'\n''    applicationHome  [[(value)]mDirectory. Required. Application home to get the version from.[[(reset)]m'$'\n'''$'\n''Extracts version from an application either from [[(code)]m.deploy[[(reset)]m files or from the the [[(code)]m.env[[(reset)]m if'$'\n''that does not exist.'$'\n''Checks [[(code)]mAPPLICATION_ID[[(reset)]m and [[(code)]mAPPLICATION_TAG[[(reset)]m and uses first non-blank value.'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m0[[(reset)]m - Success'$'\n''- [[(code)]m1[[(reset)]m - Environment error'$'\n''- [[(code)]m2[[(reset)]m - Argument error'$'\n'''
