...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]mtarExtractPattern [[(blue)]m[ pattern ]'$'\n'''$'\n''    [[(blue)]mpattern  [[(value)]mThe file pattern to extract'$'\n'''$'\n''Platform agnostic tar extract with wildcards'$'\n''e.g. tar -xf '\''[[(cyan)]m/file.json'\'' or tar -xf --wildcards '\''/file.json'\'' depending on OS'$'\n''tar command is not cross-platform so this differentiates between the GNU and BSD command line arguments.'$'\n'''$'\n''Return codes:'$'\n''- 0 - Success'$'\n''- 1 - Environment error'$'\n''- 2 - Argument error'$'\n'''$'\n''Reads from stdin:'$'\n''A gzipped-tar file'$'\n'''$'\n''Writes to stdout:'$'\n''The desired file'$'\n'''
