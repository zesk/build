...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: usageRequireEnvironment usageFunction [ environmentVariable ]'$'\n'''$'\n''    usageFunction        [[(value)]mRequired. bash function already defined to output handler'$'\n''    environmentVariable  [[(value)]mString. Optional. One or more environment variables which should be set and non-empty.'$'\n'''$'\n''Requires environment variables to be set and non-blank'$'\n'''$'\n''Return codes:'$'\n''- 0 - All environment variables are set and non-empty'$'\n''- 1 - If any environmentVariable variables are not set or are empty.'$'\n'''
