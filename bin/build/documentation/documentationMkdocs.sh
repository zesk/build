...
# shellcheck disable=SC2016
helpPlain='Usage: [[(info)]mdocumentationMkdocs [[(blue)]m[ --path documentationPath ] [[(blue)]m[ --template yamlTemplate ]'$'\n'''$'\n''    [[(blue)]m--path documentationPath  Directory. Optional. Directory where documentation root exists.'$'\n''    [[(blue)]m--template yamlTemplate   File. Optional. Name of mkdocs.yml template file to generate final file. Default is mkdocs.template.yml.'$'\n'''$'\n''Build documentation using mkdocs and a template'$'\n'''$'\n''Return codes:'$'\n''- 0 - Success'$'\n''- 1 - Environment error'$'\n''- 2 - Argument error'$'\n'''
