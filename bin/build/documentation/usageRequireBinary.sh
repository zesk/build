...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: usageRequireBinary [[(bold)]m[[(magenta)]musageFunction [[(bold)]m[[(magenta)]mbinary'$'\n'''$'\n''    [[(red)]musageFunction  [[(value)]mRequired. [[(code)]mbash function already defined to output handler'$'\n''    [[(red)]mbinary         [[(value)]mRequired. Binary which must have a [[(code)]mwhich path.'$'\n'''$'\n''Requires the binaries to be found via [[(code)]mwhich'$'\n''Runs [[(code)]mhandler on failure'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m1 - If any [[(code)]mbinary is not available within the current path'$'\n'''
