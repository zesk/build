...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]muserRecord [[(bold)]m[[(magenta)]mindex [[(blue)]m[ user ] [[(blue)]m[ database ]'$'\n'''$'\n''    [[(red)]mindex     [[(value)]mPositiveInteger. Required. Index (1-based) of field to select.'$'\n''    [[(blue)]muser      [[(value)]mString. Optional. User name to look up. Uses [[(code)]mwhoami if not supplied.'$'\n''    [[(blue)]mdatabase  [[(value)]mFile. Optional. User name database file to examine. Uses [[(code)]m/etc/passwd if not supplied.'$'\n'''$'\n''Look user up, output a single user database record.'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m0 - Success'$'\n''- [[(code)]m1 - Environment error'$'\n''- [[(code)]m2 - Argument error'$'\n'''$'\n''Writes to [[(code)]mstdout:'$'\n''String. Associated record with [[(code)]mindex and [[(code)]muser.'$'\n'''
