...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]mshaPipe [[(blue)]m[ filename ... ] [[(blue)]m[ --cache cacheDirectory ]'$'\n'''$'\n''    [[(blue)]mfilename ...            [[(value)]mFile. One or more filenames to generate a checksum for'$'\n''    [[(blue)]m--cache cacheDirectory  [[(value)]mDirectory. Cache file cache values here for speed optimization.'$'\n'''$'\n''Generates a checksum of standard input and outputs a SHA1 checksum in hexadecimal without any extra stuff'$'\n''You can use this as a pipe or pass in arguments which are files to be hashed.'$'\n'''$'\n''Return codes:'$'\n''- 0 - Success'$'\n''- 1 - Environment error'$'\n''- 2 - Argument error'$'\n'''$'\n''Reads from stdin:'$'\n''any file'$'\n'''$'\n''Writes to stdout:'$'\n''String. A hexadecimal string which uniquely represents the data in stdin.'$'\n'''$'\n''Example:'$'\n''    shaPipe < "$fileName"'$'\n''    shaPipe "$fileName0" "$fileName1"'$'\n'''$'\n''BUILD_DEBUG settings:'$'\n''- shaPipe - Outputs all requested shaPipe calls to log called shaPipe.log.'$'\n'''
