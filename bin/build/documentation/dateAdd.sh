...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]mdateAdd [[(blue)]m[ --days delta ] [[(blue)]m[ timestamp ... ]'$'\n'''$'\n''    [[(blue)]m--days delta   [[(value)]mSignedInteger. Number of days to add (or subtract - use a negative number). Affects all timestamps [[(cyan)]mafter it.'$'\n''    [[(blue)]mtimestamp ...  [[(value)]mDate. Timestamp to update.'$'\n'''$'\n''Add or subtract days from a text date'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m0 - Success'$'\n''- [[(code)]m1 - Environment error'$'\n''- [[(code)]m2 - Argument error'$'\n'''$'\n''Writes to [[(code)]mstdout:'$'\n''Date with days added to it'$'\n'''$'\n''Example:'$'\n''    newYearsEve=$(dateAdd --days -1 "2025-01-01")'$'\n'''
