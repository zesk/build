...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]mtarCreate [[(blue)]m[ target ] [[(blue)]m[ files ]'$'\n'''$'\n''    [[(blue)]mtarget  [[(value)]mThe tar.gz file to create'$'\n''    [[(blue)]mfiles   [[(value)]mA list of files to include in the tar file'$'\n'''$'\n''Platform agnostic tar cfz which ignores owner and attributes'$'\n''[[(code)]mtar command is not cross-platform so this differentiates between the GNU and BSD command line arguments without needing to know what operating system you are on. Creates a gz-compressed tar file ([[(code)]m.tgz or [[(code)]m.tar.gz) with user and group set to 0 and no extended attributes attached to the files.'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m0 - Success'$'\n''- [[(code)]m1 - Environment error'$'\n''- [[(code)]m2 - Argument error'$'\n'''
# elapsed 3.565
