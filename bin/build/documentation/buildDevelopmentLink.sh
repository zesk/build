...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]mbuildDevelopmentLink [[(blue)]m[ --copy ] [[(blue)]m[ --reset ]'$'\n'''$'\n''    [[(blue)]m--copy   [[(value)]mFlag. Optional. Copy the files instead of creating a link - more compatible with Docker but slower and requires synchronization.'$'\n''    [[(blue)]m--reset  [[(value)]mFlag. Optional. Revert the link and reinstall using the original binary.'$'\n'''$'\n''Add a development link to the local version of Zesk Build for testing in local projects.'$'\n''Copies or updates $BUILD_HOME/bin/build in current project.'$'\n''Useful when you want to test a fix on a current project.'$'\n'''$'\n''Return codes:'$'\n''- 0 - Success'$'\n''- 1 - Environment error'$'\n''- 2 - Argument error'$'\n'''
# elapsed 2.257
