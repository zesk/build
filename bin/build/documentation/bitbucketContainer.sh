...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: bitbucketContainer [[(magenta)]menvFile [[(blue)]m[ extraArgs ... ]'$'\n'''$'\n''    [[(red)]menvFile        [[(value)]mFile. Required. One or more environment files which are suitable to load for docker; must be valid'$'\n''    [[(blue)]mextraArgs ...  [[(value)]mArguments. Optional. The first non-file argument to [[(code)]mbitbucketContainer is passed directly through to [[(code)]mdocker run as arguments'$'\n'''$'\n''Run the default build container for build testing on BitBucket'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m1 - If already inside docker, or the environment file passed is not valid'$'\n''- [[(code)]m0 - Success'$'\n''- [[(code)]mAny - [[(code)]mdocker run error code is returned if non-zero'$'\n'''
