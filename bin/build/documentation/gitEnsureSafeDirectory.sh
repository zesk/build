...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]mgitEnsureSafeDirectory [[(blue)]m[ --help ] [[(bold)]m[[(magenta)]mdirectory'$'\n'''$'\n''    [[(blue)]m--help     [[(value)]mFlag. Optional. Display this help.[[(reset)]m'$'\n''    [[(red)]mdirectory  [[(value)]mDirectory. Required. The directory to add to the [[(code)]mgit[[(reset)]m [[(code)]msafe.directory[[(reset)]m configuration directive[[(reset)]m'$'\n'''$'\n''When running git operations on a deployment host, at times it'\''s necessary to'$'\n''add the current directory (or a directory) to the git [[(code)]msafe.directory[[(reset)]m directive.'$'\n''This adds the directory passed to that directory in the local user'\''s environment'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m0[[(reset)]m - Success'$'\n''- [[(code)]m2[[(reset)]m - Argument is not a valid directory'$'\n''- [[(code)]mOther[[(reset)]m - git config error codes'$'\n'''
# elapsed 3.604
