...
# shellcheck disable=SC2016
helpPlain='Usage: [[(info)]mdateFromTimestamp [[(magenta)]mintegerTimestamp [[(blue)]m[ format ] [[(blue)]m[ --help ] [[(blue)]m[ --local ]'$'\n'''$'\n''    [[(red)]mintegerTimestamp  Integer. Required. Integer timestamp offset (unix timestamp, same as [[(code)]m$(date +%s))'$'\n''    [[(red)]mformat            String. Optional. How to output the date (e.g. [[(code)]m%F - no [[(code)]m+ is required)'$'\n''    [[(blue)]m--help            Flag. Optional. Display this help.'$'\n''    [[(blue)]m--local           Flag. Optional. Show the local time, not UTC.'$'\n'''$'\n''Converts an integer date to a date formatted timestamp (e.g. %Y-%m-%d %H:%M:%S)'$'\n''dateFromTimestamp 1681966800 %F'$'\n'''$'\n''Return codes:'$'\n''- [[(code)]m0 - If parsing is successful'$'\n''- [[(code)]m1 - If parsing fails'$'\n'''$'\n''Environment variables:'$'\n''- Compatible with BSD and GNU date.'$'\n'''$'\n''Example:'$'\n''    dateField=$(dateFromTimestamp $init %Y)'$'\n'''
