...
# shellcheck disable=SC2016
helpPlain='[[(label)]mUsage: [[(info)]mdeployLink [[(bold)]mapplicationLinkPath [[(blue)]m[ applicationPath ]'$'\n'''$'\n''    [[(red)]mapplicationLinkPath  [[(value)]mPath. Required. Path where the link is created.'$'\n''    [[(blue)]mapplicationPath      [[(value)]mPath. Optional. Path where the link will point to. If not supplied uses current working directory.'$'\n'''$'\n''Link new version of application.'$'\n''When called, current directory is the [[(red)]mnew application and the applicationLinkPath which is'$'\n''passed as an argument is the place where the [[(red)]mnew application should be linked to'$'\n''in order to activate it.'$'\n'''$'\n''Return codes:'$'\n''- 0 - Success'$'\n''- 1 - Environment error'$'\n''- 2 - Argument error'$'\n'''$'\n''Environment variables:'$'\n''- PWD'$'\n'''
