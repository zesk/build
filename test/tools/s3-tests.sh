...
testAWSS3Upload() {
  ...
  local readMe="$home/bin/build/README.md"
  local buildJSON="$home/bin/build/build.json"

  local home
  home=$(catchReturn "$handler" buildHome) || return $?
  assertExitCode 0 awsS3Upload --target "$target" "$buildJSON" "$home/bitbucket-pipelines.yml" "$readMe" "$home/etc" || return $?

  local checkFile
  checkFile=$(fileTemporaryName "$handler") || return $?
  catchEnvironment "$handler" aws s3 cp "$target/manifest.json" "$checkFile" || return $?

  assertFileContains "$checkFile" "createdString" "$buildJSON" "$home/bitbucket-pipelines.yml" "$readMe" "$home/etc" || return $?
